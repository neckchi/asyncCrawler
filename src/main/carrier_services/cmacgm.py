...
def cma_mapping(crawler_result: list, network_results: list, lookup_network: list, writer: csv.DictWriter):
    carrier_code: dict = {'0001': 'CMDU', '0002': 'ANNU',
                          '0011': 'CHNL', '0014': 'CSFU', '0015': 'APLU'}
    ...
    for url, routing in zip(network_results, crawler_result):
        ...
        lookup_service_network: dict = find_dictionaries_by_value(
            lst=lookup_network, key='service_code', value=service_code)[0]
        for carrier_service in lookup_service_network['carriers']:
            scac: str = carrier_code.get(
                carrier_service['shipcomp'], 'UNKNOWN')
    ...
    with FileManager(mode='w', scac=f'{carrier}') as writer:
        ...
        for n in range(0, 700, 50):
            ...
            service_network = Crawler(
                ...
                specific_headers={
                    'range': n, 'KeyId': settings.cmdu_api_key.get_secret_value()},
                workers=4,
                limit=10000,
            )
            ...
            service_network.logging_url(
                task_name='CMA CGM Service Network Group')
        service_network_result: list = [{'service_code': data['code'], 'service_name': data['name'], 'departure_dat': data.get(
            'departureDay'), 'carriers': data['carriers']} for sn in all_services for data in orjson.loads(sn.read())]

        service_routing = Crawler(
            crawler_type='API',
            method='GET',
            sleep=1,
            urls=[settings.cmdu_route_url.format(
                loop=network['service_code']) for network in service_network_result],
