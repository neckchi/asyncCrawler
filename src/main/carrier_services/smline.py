...
def smline_mapping(crawler_result: list, network_results: list, lookup_network: list, writer: csv.DictWriter):
    for service, routing in zip(network_results, crawler_result):
        ...
        routing_direction: dict = split_list_of_dicts(
            lst=routing_list, key='skdDirCd')
...
async def smline_crawler():
    ...
    with FileManager(mode='w', scac=f'{carrier}') as writer:
        extra_header: dict = {'Host': settings.smlm_host,
                              'Origin': settings.smlm_origin, 'Referer': settings.smlm_referer}
        service_network = Crawler(
            crawler_type='API',
            method='GET',
            sleep=5,
            urls=[settings.smlm_service_url.format(
                network=network_code) for network_code in ['AA', 'MN']],
        ...
        service_network_set: list = [{'service_code': data['vslSlanCd'], 'service_name': data['vslSlanNm']}
                                     for services in service_network.result for data in orjson.loads(services.text)['list']]
        service_routing_urls: list = [settings.smlm_route_url.format(
            service=service['service_code']) for service in service_network_set]
