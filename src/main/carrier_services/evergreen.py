...
def location_mapping(location_string: str):
    ...
    for ls in list_strings:
        ...
        decoded_list: list = [[urllib.parse.unquote(
            element) for element in sublist] for sublist in list_data]
    ...
    for index, (srr, direct) in enumerate(zip(crawler_result, network_results), start=1):
        if srr:
            ...
            service_name: str = find_dictionaries_by_value(
                lst=lookup_network, key='a_link', value=f'/tvs2/jsp/TVS2_ServiceProfile.jsp?line={service_code}&segment={direction_code}')[0].get('a_text', service_code)
            direction: str = direction_lookup.get(direction_code, 'UNKNOWN')
            extended_port = list(itertools.chain(
                *[itertools.takewhile(lambda x: x != 'ETA', srr[ep + 1::]) for ep in check_extended(srr, service_code)]))
            extended_eta = list(itertools.chain(
                *[itertools.takewhile(lambda x: x != 'ETD', srr[eta + 1::]) for eta in check_extended(srr, 'ETA')]))
            extended_etd = list(itertools.chain(
                *[itertools.takewhile(lambda x: x != 'T/S TIME', srr[etd + 1::]) for etd in check_extended(srr, 'ETD')]))
            extended_tss = list(itertools.chain(
                *[itertools.takewhile(lambda x: x != service_code, srr[tss + 1::]) for tss in check_extended(srr, 'T/S TIME')]))
            for port_sequence, (port, eta, etd, tss) in enumerate(zip(extended_port, extended_eta, extended_etd, extended_tss)):
                un_location_code: str = find_dictionaries_by_value(
                    lst=lookup_location, key='location_name', value=port)[0].get('location_code')
...
async def evergreen_crawler():
    ...
    with FileManager(mode='w', scac=f'{carrier}') as writer:
        ...
        network_results: list = [service for service in service_network.result if str(
            service['a_link']).startswith('/tvs2/jsp/')]
        ...
        service_routing = Crawler(
            ...
            next_element='td',
            urls=[settings.eglv_route_url.format(
                result['a_link']) for result in network_results],
            sleep=4,
            workers=3,
            limit=5000,
        )
        ...
        location_name: set = set(itertools.chain(*[itertools.takewhile(lambda x: x != 'ETA', loc[ep + 1::])
                                 for loc in service_routing.result for ep in check_extended(service_list=loc, element=loc[0])]))

        call_location_code = Crawler(
            crawler_type='API',
            method='GET',
            sleep=4,
            urls=[settings.eglv_location_url.format(
                port=loc_name) for loc_name in location_name],
            workers=3,
            limit=20000,
        )
