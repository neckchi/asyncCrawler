...
class Crawler():
    ...
    def __init__(self, urls: Iterable[str], crawler_type=Literal['API', 'Web'], method=Literal['GET', 'POST'],
        ...
        limits = httpx.Limits(
            max_keepalive_connections=60, max_connections=None)
    ...
    async def run(self):
        ...
        workers = [asyncio.ensure_future(self.worker())
    ...
    async def crawl(self, url: str):
        headers: dict = Crawler.get_random_header(
            header_list=browser_headers.headers_list)
        ...
        if self.method == 'GET':
            ...
            body_data: dict | None = {key: value[0] for key, value in parse_qs(
                parsed_url.query).items()} if parsed_url.query else None
    ...
    async def parse_links(self, base_url: str, response: httpx.Response) -> set[str]:
        if self.type == 'API':
            ...
            parsed_soup = soup.findAll(self.parent_element, class_=self.html_class) if self.html_class else soup.findAll(
                self.parent_element, id=self.html_id)
            if self.element == 'ahref':
                ...
                self.result.append([service.text.strip()
                                   for service in parsed_soup])
            else:
                self.result.append(parsed_soup)
